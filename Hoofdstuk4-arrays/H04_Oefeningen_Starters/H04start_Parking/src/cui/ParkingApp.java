...
public class ParkingApp {
	...
	private void parkeer(Parking parking) {
		String nummerplaat = geefNummerplaat();
		int indexParkeerPlaats = parking.parkeer(nummerplaat);
		if(indexParkeerPlaats == -1) {
			System.out.printf("Parkeerplek staat vol");
		}
		else {
			System.out.printf("De wagen met nummerplaat %s werd geparkeerd op plaats %d.%n", nummerplaat, indexParkeerPlaats);
		}
	}

	private void verlaat(Parking parking) {
		String nummerplaat = geefNummerplaat();
		boolean plaatsVerlaten = parking.verlaat(nummerplaat);
		if(plaatsVerlaten) {
			System.out.printf("De wagen met nummerplaat %s werd uit de parking gereden.%n", nummerplaat);
		}
		else {
			System.out.printf("De wagen met nummerplaat %s staat niet in de parking.%n", nummerplaat);
		}
	}
